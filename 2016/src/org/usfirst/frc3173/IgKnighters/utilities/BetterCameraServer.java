...
public class BetterCameraServer {
  ...
  //private static BetterCameraServer server;

  /*public static BetterCameraServer getInstance(int port) {
    if (server == null) {
      server = new BetterCameraServer(port);
    }
    return server;
  }*/
  ...
  public BetterCameraServer(int port) {
	  kPort=port;
    m_quality = 50;
    m_camera = null;
    m_imageData = null;
    ...
    serverThread.setName("CameraServer Send Thread 2");
    serverThread.start();
  }
  ...
  public synchronized void startAutomaticCapture(USBCamera camera) {
    ...
    captureThread.setName("Camera Capture Thread 2");
    captureThread.start();
  }
  ...
  protected void capture() {
    ...
    while (true) {
      ...
      try {
    	...
    	//SmartDashboard.putBoolean("capture", isAutoCaptureStarted());

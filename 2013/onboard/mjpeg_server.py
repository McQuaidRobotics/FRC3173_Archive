...
class IPCameraApp(object):
    ...
    def __call__(self, environ, start_response):    
        ...
        elif environ['PATH_INFO'] == '/mjpg/video.mjpg':
    ...
    def stream(self, start_response):
        ...
        # Display "Loading..." message (Added by Wolfgang Faust)
        imageData=open('igknighters.jpg').read()
        q.put("Content-type: image/jpeg\n"+"Content-length: "+str(len(imageData))+"\n\n"+imageData+"\n\n--spionisto\n");
...
def input_loop(app):
    sock = socket()
    sock.bind(('localhost', 9999)) # Binding to localhost bypasses several network layers
    sock.listen(1)
    while True:

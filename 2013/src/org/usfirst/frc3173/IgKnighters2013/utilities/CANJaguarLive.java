 ...
 * wrapper method. All JavaDoc for wrapper methods is copied from the WPI 
 * CANJaguar code.
 * @author Wolfgang Faust
 */
public class CANJaguarLive implements CANJaguar {
	/**
	 ...
	 */
	public CANJaguarLive (int id, boolean slowRamp) {
		m_id = id;
		instantiate();
		if (slowRamp) {
   ...
   /**
	...
	public void changeControlMode(edu.wpi.first.wpilibj.CANJaguar.ControlMode mode) {
		try {
			m_jaguar.changeControlMode(mode);
                        if(mode==edu.wpi.first.wpilibj.CANJaguar.ControlMode.kPosition){
                            cmode=true;
                        }
			return;
		} catch (CANTimeoutException e) {
	...
	public edu.wpi.first.wpilibj.CANJaguar.ControlMode getControlMode() {
		try {
    ...
    public void setSpeedReference(SpeedReference ref) {
        try {
            m_jaguar.setSpeedReference(ref);
        } catch (CANTimeoutException ex) {
            ex.printStackTrace();
        }
    }
